...
The solution above worked well until I tested on an element that has more than one line. This is problematic considering that multi-line element is common in JSX. It would not work as expected because we are <strong>searching only on the line where the cursor is</strong>, and because there is <strong>no appropriate row number to refer to when setting the cursor</strong>.
